// dsh-imagegen — image_gen and image_vision model tools for the DeepSeek Harness.
//
// Both tools are powered by the ChatGPT login state (Codex CLI auth) through
// the harness-owned transports in ./scripts/:
//
//   1. image_gen    — generate a bitmap image via the Codex backend
//                     (scripts/codex-imagegen.mjs). Falls back to the OpenAI
//                     Images API (scripts/image_gen.py, gpt-image-2) only when
//                     an OPENAI_API_KEY credential is configured.
//   2. image_vision — describe or answer questions about an image via the
//                     Codex backend (scripts/codex-vision.mjs, multimodal
//                     gpt-5.5), giving text-only models plug-in vision.
//
// Auth precedence (both transports): CODEX_* environment variables passed by
// this plugin (resolved from the DSH credentials service) > ~/.codex/auth.json
// (`codex login`). No OpenAI API key is required.
//
// Install: place this file and the scripts/ directory inside an agent preset
// directory (e.g. ~/.dsh/.agent-presets/<id>/), then append a composition row:
//
//   - id: tool-imagegen
//     name: './imagegen-tool.js'
//
// or run ./install.sh from this repository.
...
export default {
  ...
  apply(ctx) {
    ...
    const visionTool = harness.defineTool({
      name: 'image_vision',
    ...
    ctx.effect(() => harness.registerTool(ctx, genTool), 'dsh-imagegen: register image_gen')
    ctx.effect(() => harness.registerTool(ctx, visionTool), 'dsh-imagegen: register image_vision')
  }
}
